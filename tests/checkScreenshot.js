const { chromium, webkit, firefox, devices, expect } = require('@playwright/test');
    ...
    for (const browserType of [webkit, chromium, firefox]) {//перебираем по всем браузерам
        ...
        for (const url of ['https://abrosko-studio.ru/']) {
            ...
            if (nameofpage == '/') {
                ...
                expect (await page.screenshot()).toMatchSnapshot(`currentscreenshot/screenshot${browserType.name()}.png`);
                // await expect(page).toHaveScreenshot({ path: `currentscreenshot/screenshot${browserType.name()}.png`, fullPage: true });
            }
            else {
                expect (await page.screenshot()).toMatchSnapshot(`currentscreenshot/screenshot${nameofpage}-${browserType.name()}.png`);
                // await expect(page).toHaveScreenshot({ path: `currentscreenshot/screenshot${nameofpage}-${browserType.name()}.png`, fullPage: true });
            }
